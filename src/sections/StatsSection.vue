...
onMounted(() => {
  const ctx = gsap.context(() => {
    ...
    // 2. Animasi Bar & Counter
    skills.forEach((skill, index) => {
      // Timeline per skill agar sinkron
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: sectionRef.value,
          start: "top 75%",
          toggleActions: "play reverse play reverse", // Animasi jalan lagi saat scroll
        },
      });
      ...
      // Kita gunakan objek proxy untuk animasi angka agar reaktivitas Vue tidak "bingung"
      // Tapi karena ini DOM langsung, innerText via GSAP bisa jalan asal selector benar.
      // Selector class="counter-{{ index }}" di template harus match
      tl.fromTo(
        `.counter-${index}`,
        { innerText: 0 },
        {
          ...
          duration: 1.5,
          snap: { innerText: 1 }, // Snap ke bilangan bulat
          ease: "power2.out",
          onUpdate: function () {
            // Force update text content during animation if needed
...
<template>
  ...
  >
    ...
    ></div>
    <div
      class="absolute -right-20 bottom-0 w-64 h-64 bg-secondary/10 rounded-full blur-[120px]"
    ></div>
    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
      <div class="grid lg:grid-cols-2 gap-16 items-center">
        <div class="skill-text space-y-6">
          <div
            class="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-secondary/30 bg-secondary/5 text-secondary text-xs font-mono uppercase tracking-widest"
          >
            <span
          ...
          <h2
            class="text-4xl lg:text-5xl font-display font-bold text-white leading-tight"
          >
          ...
          <p class="text-slate-400 text-lg leading-relaxed">
            Dakwah hari ini bukan sekadar ceramah di atas mimbar, tapi bagaimana
            pesan itu sampai ke
            <strong class="text-white">beranda media sosial</strong> dan
            <strong class="text-white">hati generasi muda</strong>. Saya
            menggabungkan nilai Islam dengan kreativitas visual.
          </p>
          ...
          >
            <div class="flex justify-between items-end mb-2">
              <div class="flex items-center gap-3">
                <span
                  class="material-symbols-outlined text-secondary/80 text-xl"
                  >{{ skill.icon }}</span
                >
                <div>
                  <h4 class="text-white font-bold font-display tracking-wide">
                    {{ skill.name }}
                  </h4>
                  <p
                    class="text-[10px] text-slate-500 uppercase tracking-wider hidden sm:block"
                  >
                    {{ skill.desc }}
                  </p>
                </div>
              ...
              <div class="text-right">
                <span
                  :class="`text-xl font-mono font-bold text-secondary counter-${index}`"
                  >0</span
                >
            ...
            <div
              class="h-3 w-full bg-surface-light rounded-full overflow-hidden border border-white/5 relative"
            >
              <div
              ...
              >
                <div
                  class="absolute right-0 top-0 bottom-0 w-[2px] bg-white shadow-[0_0_10px_white]"
                ></div>
              </div>
            </div>
          </div>
